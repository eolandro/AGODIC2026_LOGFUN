...
"""
Bada Boom!!! Parte 1
define una funcion en python que dado un numero entero
 te devuelva un resultado segun las siguientes condiciones:
...
def decodifica(LA, LB):
    ...
    b, *restoB = LB
    valor = a * 16 + b
    return chr(valor) + decodifica(restoA, restoB)
...
def composicion_recursiva(Valor, Funciones):
    if not Funciones:
        return Valor
    primera, *resto = Funciones
    return composicion_recursiva(primera(Valor), resto)
def fun1(cadena):
    return cadena.split(",")
def fun2(partes):
...
R = "29590095,ROMAY TACITURNO RENE FERNANDA,0,1,1,1,1,1,1,0"
T = composicion_recursiva(R, [fun1, fun2, fun3])
print(T)
